...
const assignedActivityIds = computed(() => {
  // Get all activity IDs that are assigned to any of the selected students
  const allAssignedIds = new Set();
  
  props.selectedStudents.forEach(studentId => {
    const student = props.students.find(s => s.id === studentId);
    if (student && student.activity_responses) {
      student.activity_responses
        .filter(r => r.status !== 'removed')
        .forEach(r => allAssignedIds.add(r.activity_id));
    }
  });
...
<style scoped>
/* Inherits base modal styles from AssignModal */
